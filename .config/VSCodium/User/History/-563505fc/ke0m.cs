...
public class DetectConeComponent : MonoBehaviour
{
     ...
     void FixedUpdate()
    {
        ...
        Vector3 nuevaDireccion = rotacion * direccionOriginal;
        if (Physics.Raycast(transform.position, fwd, _maxDistance ,_targetLayer)
        ||Physics.Raycast(transform.position, fwd, _maxDistance ,_targetLayer)||
            Physics.Raycast(transform.position, transform.rotation, _maxDistance ,_targetLayer)){
