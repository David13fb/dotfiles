...
public class DetectConeComponent : MonoBehaviour
{
     ...
     void FixedUpdate()
    {
        ...
        ||Physics.Raycast(transform.position, dirPos, _maxDistance ,_targetLayer)||
            ...
            Debug.DrawRay(transform.position, transform.TransformDirection(dirPos) * _maxDistance, Color.yellow);
            Debug.DrawRay(transform.position, transform.TransformDirection(dirNeg) * _maxDistance, Color.yellow);
            print("There is something in front of the object!");
        }
        else{
            Debug.DrawRay(transform.position, transform.TransformDirection(Vector3.forward) * _maxDistance, Color.white);
            Debug.DrawRay(transform.position, transform.TransformDirection(dirPos) * _maxDistance, Color.white);
            Debug.DrawRay(transform.position, transform.TransformDirection(dirNeg) * _maxDistance, Color.white);
        }
    }
}
