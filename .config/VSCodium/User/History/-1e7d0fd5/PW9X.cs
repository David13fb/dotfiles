        ...
        if(_bot != null)
        {
            ...
            foreach (GameObject aux in NPCList)
            {
                if (aux != null && _bot.NavMeshAgent.isOnNavMesh)
                {
                    ...
                    closestDistance = GetPathLength(path); 
                    if (GetPathLength(path) < _targetNPCDis)
                    {
                        _targetNPCDis = GetPathLength(path);
                        _targetNPC = aux;
                    }
                }
            }
            ...
            foreach (GameObject aux in enemyList)
            {
                if (aux != null && _bot.NavMeshAgent.isOnNavMesh)
                {
                    ...
                    _bot.NavMeshAgent.CalculatePath(hit.position, path);
                    closestDistance = GetPathLength(path); 
                    if (GetPathLength(path) < _targetEnemyDis)
                    {
                        _targetEnemyDis = GetPathLength(path);
                        _targetEnemy = aux;
                    }
                }
            }
            ...
            foreach (GameObject aux in weaponList)
            {
                if (aux != null && _bot.NavMeshAgent.isOnNavMesh)
                {
                    ...
                    _bot.NavMeshAgent.CalculatePath(hit.position, path);
                    closestDistance = GetPathLength(path); 
                    if (GetPathLength(path) < _targetWeaponDis)
                    {
                        _targetWeaponDis = GetPathLength(path);
                        _targetWeapon = aux;
                    }
                }
            ...
            foreach (GameObject aux in healthList)
            {
                if (aux != null && _bot.NavMeshAgent.isOnNavMesh)
                {
                    ...
                    _bot.NavMeshAgent.CalculatePath(hit.position, path);
                    closestDistance = GetPathLength(path); 
                    if (GetPathLength(path) < _targetHealthDis)
                    {
                        _targetHealthDis = GetPathLength(path);
                        _targetHealth = aux;
                    }
                }
            }
