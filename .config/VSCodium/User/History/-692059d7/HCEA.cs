...
namespace UCM.IAV.Navegacion
{
    ...
    public class TheseusGraph : MonoBehaviour
    {
        ...
        protected Vertex actPlayerVertex;

        protected Vertex nextPlayerVertex;
        protected ControlJugador controlJugador;

        protected LineRenderer hilo;
        protected float hiloOffset = 0.2f;

        [SerializeField] Metricas _metricas;
        ...
        public virtual void Awake()
        {
            ...
            ariadna = false;
            controlJugador = srcObj.GetComponent<ControlJugador>();
            playerTrans = srcObj.GetComponent<Transform>();

            hilo.startWidth = 0.15f;
            hilo.endWidth = 0.15f;
            hilo.positionCount = 0;
            actPlayerVertex = new Vertex();
            nextPlayerVertex = null;
        }
        ...
        public virtual void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.S))
            {
                ...
                string texto = smoothPath?"Sí" : "No";
                smoothText.text = texto;
            }
            Vertex auxVertex = graph.GetNearestVertex(srcObj.transform.position);
            if (ariadna && path.Count <= 0)
            {
                ...
                if (auxVertex != actPlayerVertex && nextPlayerVertex != actPlayerVertex)
                {
                    actPlayerVertex = auxVertex;
                    switch (algorithm)
                    {
                        ...
                        _stopwatch.Start();
                            if (firstHeuristic) path = graph.GetPathAstar(srcObj, dstObj, Graph.HeuristicEuclidean); // COMO SEGUNDO ARGUMENTO SE DEBERÍA PASAR LA HEURÍSTICA
                            else path = graph.GetPathAstar(srcObj, dstObj, Graph.HeuristicManhattan); // COMO SEGUNDO ARGUMENTO SE DEBERÍA PASAR LA HEURÍSTICA
                         _stopwatch.Stop();
                         _metricas.setTime(_stopwatch.ElapsedMilliseconds);
                         _metricas.setIt(path.Count.ToString());
                          _metricas.setlong((path.Count*graph.cellSize).ToString());
                          visitadas = 0;
                          _metricas.setVisit(visitadas.ToString());
                          float auxCost = 0;
                          foreach(var a in path){ auxCost += a.cost;
                        print(a.cost);
                          } 
                          _metricas.setCost(auxCost.ToString());
                            break;
                        default:
            ...
            {
                DibujaHilo();
                if (nextPlayerVertex == auxVertex || nextPlayerVertex == null)
                {
                    visitadas++;
                ...
                if (GetNextNode() != null)
                {
                    controlJugador.setAriadna(ariadna, GetNextNode().position - playerTrans.position);
                    actPlayerVertex = auxVertex;
                    nextPlayerVertex = path[path.Count - 1];
                   
                }
                else
                {
                    controlJugador.setAriadna(ariadna, Vector3.zero);
                    path.Clear();
                    actPlayerVertex = null;
                    nextPlayerVertex = null;
                }
            }
            else
            {
                srcObj.GetComponent<ControlJugador>().setAriadna(ariadna, Vector3.zero);
                path.Clear();
            }
        }
