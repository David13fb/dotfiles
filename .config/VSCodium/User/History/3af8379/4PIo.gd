...
func initround(_numbaits : int) -> void:
	numbaits = _numbaits
	animator.play(buoy_idle.resource_name)
...
func bitetime() -> void :
	...
	if numbaits == 0:
		truebite.emit()
		animator.play(bite_anim.resource_name)
	else:
		animator.play(bait_array[randi_range(0,bait_array.size()-1)].resource_name)
