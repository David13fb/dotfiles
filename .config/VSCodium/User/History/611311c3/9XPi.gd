...
func show_dialogue(event_name: String):
	show_dialogue_battle.emit()
	if event_name.begins_with("F"):
		fail.emit()
	else:
		last_node_id = event_name
...
func show_next_dialogue():
	...
	AudioManager.play_sfx("res://Assets/Sfx/dialogo.wav")
	...
	if not is_end_of_dialogue:
		...
		var a: bool = last_node_id.begins_with("E");
		if a:
			dialog_end.emit()
		if last_node_id.begins_with("Carta"):
			dialog_end.emit()
		if last_node_id.begins_with("Espejo"):
			dialog_end.emit()
