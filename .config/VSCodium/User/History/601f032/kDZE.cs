...
public class TargetListControllerCmp : MonoBehaviour
{
    ...
    // Calcula el camino entre el inicio y el destino
    if (NavMesh.CalculatePath(start, end, NavMesh.AllAreas, path)) {
        if (path.status == NavMeshPathStatus.PathComplete) {
            float distance = 0;
            // Suma la distancia entre cada "esquina" del camino
    ...
    return float.MaxValue; // Si no hay camino válido, devolvemos un valor infinito
}
