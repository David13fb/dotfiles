...
/// </para>
/// <para>
/// Autoridad: se pretende que el servidor sea la única fuente de verdad (ver <see cref="ExportarDatos"/>).
/// </para>
/// <para>
...
public class MatchDataManager : MonoBehaviour
{
    ...
    /// </remarks>
    public MatchDataWrapperExport ExportarDatos()
    {
        if (NetworkManager.Singleton != null && !NetworkManager.Singleton.IsServer) return new MatchDataWrapperExport();
        if (playersData.Count == 0) return new MatchDataWrapperExport();
        ...
        return wrapperExport;
        /*
        string json = JsonUtility.ToJson(wrapperExport, true);
        string timeStamp = DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss");
        string filePath = Path.Combine(Application.dataPath+ "resultados" + Path.DirectorySeparatorChar, $"ResultadosPartida_{timeStamp}.json");
