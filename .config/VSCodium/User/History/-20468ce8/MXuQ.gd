...
var monedasP1 : Array[Node]
var monedasP2 : Array[Node]
...
func resetRound() :
	...
	monedasP1 = puntosP1.get_children()
	print(monedasP1.size())
	monedasP2 = puntosP2.get_children()
...
func _process(_delta: float) -> void:
	...
	if Game_Manager.getTime() <= 0 or P1miss and P2miss:
		...
		for i in range(Game_Manager.PointsP1 ):
			monedasP1[i].visible = true
		...
		InitMiniGame.btn_input.visible = false
		GanadorFinal.text = "PEZ PARA: JUGADOR 2" 
		for i in range(Game_Manager.PointsP2):
			monedasP2[i].visible = true		
		audio_p1.stream = sound_success
		audio_p1.play()
		GanadorFinal.visible = true
